...
class DCGAN(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            real_fake_combined_output = tf.concat([real_output,fake_output], axis=0)#concatenated real and fake output
            labels = tf.concat([tf.ones_like(real_output), tf.zeros_like(fake_output)], axis=0)#labels defined 0 and 1
        
            g_loss = self.loss_fn(tf.ones_like(fake_output),fake_output)
            d_loss = self.loss_fn(labels,real_fake_combined_output)

        gradients_of_generator = gen_tape.gradient(g_loss, self.generator.trainable_variables)
        gardients_of_discriminator = disc_tape.gradient(d_loss, self.discriminator.trainable_variables)


        self.g_optimizer .apply_gradients(zip(gradients_of_generator,self.generator.trainable_variables))
        self.d_optimizer .apply_gradients(zip(gardients_of_discriminator,self.discriminator.trainable_variables))
...
def train_dcgan_mnist():
    ...
    # train the DCGAN
    dcgan.fit(x_train, epochs=100, batch_size=64, callbacks=callbacks, shuffle=True)
